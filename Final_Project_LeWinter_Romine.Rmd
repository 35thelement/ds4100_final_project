...
# Summary Statistics
```{r}
emigrants <- call_neo4j('MATCH (c:Country)-[:EMMIGRATION]->(m:Migration) RETURN c.name AS Name, sum(m.total) AS Total_Emigrations, sum(m.male) AS Total_Male_Emigrants, sum(m.female) AS Total_Female_Emigrants', con) %>%
  data.frame()
colnames(emigrants) <- c("Country Name", "Total Emigrants", "Total Male Emigrants", "Total Female Emigrants")
immigrants <- call_neo4j('MATCH (c:Country)<-[:IMMIGRATION]-(m:Migration) RETURN c.name AS Name, sum(m.total) AS Total_Immigrations, sum(m.male) AS Total_Male_Immigrants, sum(m.female) AS Total_Female_Immigrants', con) %>%
  data.frame()
colnames(immigrants) <- c("Country Name", "Total Immigrants", "Total Male Immigrants", "Total Female Immigrants")
migration_totals <- merge(emigrants, immigrants)
ggplot(data = migration_totals) + geom_point(aes(x = `Total Emigrants`, y = `Total Immigrants`))
ggplot(data = migration_totals) + geom_point(aes(x = `Total Emigrants`, y = `Total Immigrants`))
```
