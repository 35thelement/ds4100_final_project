...
To begin with, let's see the total numbers of emigrants and immigrants of each kind of income level for all countries over time:
```{r}
emigrants_income <- call_neo4j('MATCH (i:Income)<-[:INCOME]-(c:Country)-[:EMMIGRATION]->(m:Migration)-[:IN_YEAR]->(y:Year) RETURN i.name AS Income, y.year AS Year, sum(m.total) AS Total_Emigrants', con) %>%
  data.frame()
colnames(emigrants_income) <- c("Income Level", "Year", "Total Emigrants")
immigrants_income <- call_neo4j('MATCH (i:Income)<-[:INCOME]-(c:Country)<-[:IMMIGRATION]-(m:Migration)-[:IN_YEAR]->(y:Year) RETURN i.name AS Income, y.year AS Year, sum(m.total) AS Total_Emigrants', con) %>%
  data.frame()
colnames(immigrants_income) <- c("Income Level", "Year", "Total Immigrants")
...
Now, let's try and perform a linear regression to predict the number of immigrants and emigrants from each levels of income of countries in 2020:
...
Having done that, let's do the same thing again, except this time we'll be looking at migration by countries' regions instead of their income levels:
```{r}
emigrants_region <- call_neo4j('MATCH (r:Region)<-[:REGION]-(c:Country)-[:EMMIGRATION]->(m:Migration)-[:IN_YEAR]->(y:Year) RETURN r.name AS Region, y.year AS Year, sum(m.total) AS Total_Emigrants', con) %>%
  data.frame()
colnames(emigrants_region) <- c("Region", "Year", "Total Emigrants")
immigrants_region <- call_neo4j('MATCH (r:Region)<-[:REGION]-(c:Country)<-[:IMMIGRATION]-(m:Migration)-[:IN_YEAR]->(y:Year) RETURN r.name AS Region, y.year AS Year, sum(m.total) AS Total_Immigrants', con) %>%
  data.frame()
colnames(immigrants_region) <- c("Region", "Year", "Total Immigrants")
...
Now, let's try and perform a linear regression to predict the number of immigrants from each region in 2020:
